class Solution {
    public int largestOverlap(int[][] img1, int[][] img2) {
        ...
        for(int row = -img1.length + 1; row < img1.length; row++){
            for(int col = -img2.length + 1; col < img2.length; col++){
                largestOverlap = Math.max(largestOverlap, overLapsOnes(img1,img2,row,col));
            }
        }
    ...
    public int overLapsOnes(int[][] img1, int[][] img2, int rowOff, int colOff){
        int count = 0;
